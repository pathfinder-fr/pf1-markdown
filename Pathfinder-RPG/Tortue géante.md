---
Name: Pathfinder-RPG.Tortue géante
Title: Tortue, Tortue géante
LastModified: 2020-09-22 16:42
Categories:
- Monstre
- Règle officielle
- Src Bestiaire 4
---

<table class="Bestiaire"><tr><td>
<div class="BD">
{s:BDTitre|Tortue géante|FP 1}
{s:pucem|Animal|Désert|Tempéré}
- *Source : Bestiaire 4, p.273*
- **PX** 400
- [[type Animal|Animal]] de taille M, N
- **Init** -2 ; **Sens** [[vision nocturne]] ; [[Perception]] +7
(((Défense)))
- **CA** 16, contact 8, pris au dépourvu 16 (Dex -2, naturelle +8)
- **pv** 22 (3d8+9)
- **Réf** +1, **Vig** +8, **Vol** +2
(((Attaque)))
- **VD** 3 m (2 {s:c})
- **Corps à corps** morsure, +4 (1d6+3)
(((Statistiques)))
- **For** 14, **Dex** 7, **Con** 16, **Int** 2, **Sag** 13, **Cha** 9
- **BBA** +2 ; **BMO** +4 ; **DMD** 12 (16 contre [[croc-en-jambe]])
- **Dons** [[Endurance]], [[Vigueur surhumaine]]
- **Compétences** [[Perception]] +7 (+3 pour les tests basés sur le son)
- **Modificateurs raciaux** -4 aux tests basés sur le son
- **Particularités** flottable, carapace, lentement mais sûrement.
(((Écologie)))
- **Environnement** déserts ou îles chaudes
- **Organisation sociale** solitaire, couple ou troupeau (6-12)
- **Trésor** aucun
(((Pouvoirs spéciaux)))
- **Carapace (Ext).** Par une action de mouvement, une tortue peut dissimuler
ses membres et sa tête sous sa carapace. Elle ne peut plus se déplacer ou
attaquer tant qu’elle reste dans cette position, mais son bonus d’armure
naturelle augmente de 4 points. Elle peut mettre fin à cet état par une
action de mouvement.
- **Flottable (Ext).** Une tortue géante qui rate un test de Natation avec une marge d’échec de 5 ou plus ne coule pas.
- **Lentement mais sûrement (Ext).** La vitesse de déplacement d’une
tortue géante n’est jamais modifiée par le port d’une armure ou la
charge transportée.

</div>
</td><td><div><div style="float:right; background-color: #fff; padding: 0 0 0 0; margin: 0 0 4px 8px">
{| CLASS="tablo autoalt"
|+ Variantes de tortue
|- CLASS="titre"
| Nom || FP
|- CLASS="premier"
| [[Tortue géante]] || 1
|-
| [[Tortue immense]] || 8
|}
</div>*Cette tortue est moitié moins grande qu’un humain et possède une carapace en dôme, un cuir tanné et un long cou.*

Les tortues géantes vivent généralement sur des
îles tropicales dénuées de grands prédateurs.
Malgré leur carapace impressionnante, leur lenteur
en fait des proies faciles, surtout pour les
diverses vermines et autres créatures qui apprécient
leurs œufs. Les tortues sont herbivores
et peuvent survivre plusieurs mois, voire plusieurs
années, sans nourriture ni eau. Elles
nagent avec difficulté, mais leur flottabilité
et leur long cou leur permettent de survivre
relativement longtemps en mer.

Une tortue géante typique fait un mètre
de haut environ du sommet de sa carapace
pour un poids de 250 kilogrammes.

### La tortue géante comme compagnon animal
**Caractéristiques de départ.** **Taille** M ; **Vitesse** 3 m (2 {s:c}) ; **CA** armure naturelle +6 ; **Attaque** morsure (1d4) ; **Caractéristiques** For 14, Dex 8, Con 16, Int 2, Sag 13, Cha 9 ; **Particularités** [[vision nocturne]], flottable, carapace, lentement mais sûrement.

**Évolution au niveau 7.** **Taille** G ; **CA** armure naturelle +8 ; **Attaque** morsure (1d6) ; **Caractéristiques** For +4, Dex –2, Con +2 ; **Don supplémentaire** [[Vigueur surhumaine]].

*<u>Note :</u> Dans certaines V.F. et V.O. les particularités évidentes "flottable", "carapace", "lentement mais sûrement" semblent avoir été oubliées sur la version compagnon animal. Cela devrait avoir été corrigé depuis dans les dernières versions.*
</div></td></tr></table>
