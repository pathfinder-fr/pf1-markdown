...
Les modèles suivants représentent des monstres originaux ainsi que la faune des plans intérieurs, comme les plans élémentaires ou le Premier Monde des Fées, et servent la même fonction que les archétypes simples [Céleste](http://www.pathfinder-fr.org/Wiki/Pathfinder-RPG.Arch%c3%a9types.ashx#Créature_céleste_FP_+A_ou_+B_1) et [Fielon](http://www.pathfinder-fr.org/Wiki/Pathfinder-RPG.Arch%c3%a9types.ashx#Créature_fiélon_FP_+A_ou_+B_4). Pour chaque modèle ci-dessous, les « règles rapides » et les « règles de reconstruction » sont les mêmes.
...
**Règles de reconstruction**. **Type** la créature gagne le sous-type [[sous-type air|Air]] ; **Sens** ajouter [[vision dans le noir]] à 18 m (12 {s:c}) ; **Capacités défensives** ajouter une résistance à l'électricité et la [[RD|Réduction de Dégâts]] comme indiqué sur le tableau  ; **Déplacement** ajouter une vitesse de vol égale à la vitesse de déplacement la plus haute de la créature avec une manœuvrabilité parfaite (maximum 3 m/D.V.) ; **Attaques** les armes naturelles et manufacturées en métal infligent des dégâts d’électricité supplémentaires comme indiqué sur le tableau.
...
**Règles de reconstruction**. **Type** la créature gagne le sous-type [[sous-type eau|Eau]] et est donc considérée comme étant amphibie ; **Sens** ajouter [[vision dans le noir]] à 18 m (12 {s:c}) ; **Capacités défensives** ajouter une résistance au froid et la [[RD|Réduction de Dégâts]] comme indiqué sur le tableau  ; **Déplacement** ajouter une vitesse de nage égale à la vitesse de déplacement la plus haute de la créature + 3 m (2 {s:c}) ; **Attaques** les armes naturelles et manufacturées en métal infligent des dégâts de froid supplémentaires comme indiqué sur le tableau.
...
**Règles de reconstruction**. **Type** la créature gagne le sous-type [[sous-type terre|Terre]] ; **Sens** ajouter [[vision dans le noir]] à 18 m (12 {s:c}) ; **Capacités défensives** ajouter une résistance à l'acide et la [[RD|Réduction de Dégâts]] comme indiqué sur le tableau  ; **Déplacement** ajouter une vitesse de creusement égale à la moitié de la vitesse de déplacement la plus haute de la créature (ses tunnels s'effondrent automatiquement derrière elle en ne laissant jamais de passage utilisable) ; **Attaques** les armes naturelles infligent des dégâts d’acide supplémentaires comme indiqué sur le tableau.
...
**Règles de reconstruction**. **Type** la créature gagne le sous-type [[sous-type feu|Feu]] et devient donc immunisée contre le feu et vulnérable au froid ; **Sens** ajouter [[vision dans le noir]] à 18 m (12 {s:c}) ; **Capacités défensives** ajouter la [[RD|Réduction de Dégâts]] comme indiqué sur le tableau  ; **Attaques** les armes naturelles et manufacturées en métal infligent des dégâts de feu supplémentaires comme indiqué sur le tableau.
