---
Name: Pathfinder-RPG.Hydromètre géant
Title: Hydromètre, Hydromètre géant
LastModified: 2020-09-20 00:19
---

<table class="Bestiaire"><tr><td>
<div class="BD">
{s:BDTitre|Hydromètre géant|FP 1}
{s:pucem|Vermine|Aquatique|Tempéré}
{s:BDTexte|*Source : Bestiaire 4 page 147*}
- **400 PX**
- [Vermine](./type Vermine.md) de taille G, N
- **Init **+1 ; **Sens** vision dans le noir 18 m, perception des vibrations 18 m ; Perception +4
(((Défense)))
- **CA** 12, contact 10, pris au dépourvu 11 (Dex +1, naturelle +2, taille -1)
- **pv** 19 (3d8+6)
- **Réf **+2, **Vig **+5, **Vol **+1
- **Immunités** effets mentaux
(((Attaque)))
- **VD** 9 m, vol 6 m (médiocre)
- **Corps à corps** 2 griffes, +2 (1d4+1) et morsure, +2 (1d2+1)
- **Espace** 3 m ; **Allonge** 1,50 m
(((Statistiques)))
- **For** 13, **Dex** 13, **Con** 14, **Int** -, **Sag** 10, **Cha** 2
- **BBA **+2 ; **BMO **+4 ; **DMD** 16 (24 contre le croc-en-jambe)
- **Compétences** [Natation](./Natation.md) +5, [Perception](./Perception.md) +4, [Vol](./Vol.md) -5 ; **Modificateurs raciaux** +4 en Perception, +4 en Natation
- **Particularités** marche sur l’eau
(((Écologie)))
- **Environnement** étendues d’eau
- **Organisation** solitaire ou colonie (2-9)
- **Trésor** fortuit
(((Pouvoirs spéciaux)))
- **Marche sur l’eau (Ext).** Un hydromètre peut se déplacer sur la surface d’une étendue d’eau comme s’il marchait sur la terre ferme. Un hydromètre qui nage peut se mettre debout sur la surface de l’eau en réussissant un test de Natation.
- **Perception des vibrations (Ext)**. La perception des vibrations d’un hydromètre fonctionne uniquement lorsque la créature est en contact avec la surface d’une étendue d’eau.

</div>
</td><td><div><div style="float:right; background-color: #fff; padding: 0 0 0 0; margin: 0 0 4px 8px">
{| CLASS="tablo autoalt"
|+ Variantes d'hydromètre
|- CLASS="titre"
| Nom || FP
|- CLASS="premier"
| [Hydromètre géant](./Hydromètre géant.md) || 1
|-
| [Nymphe d'hydromètre](./Nymphe dhydromètre.md) || 1/4
|}
</div>*Cet insecte est doté de longues pattes duveteuses qui lui permettent de marcher sur l’eau tout aussi facilement que sur la terre ferme.*

Un hydromètre géant est un insecte aux longues pattes capable de marcher sur l’eau. Il se nourrit de poissons et d’oiseaux. Les hydromètres utilisent leur faible capacité de vol uniquement pour échapper aux prédateurs ou pour effectuer leurs migrations saisonnières vers de nouvelles zones de reproduction. Les humanoïdes qui vivent près des côtes les dressent parfois pour en faire des montures. Un hydromètre géant vit 1 à 2 ans.

Si un hydromètre géant porte une charge supérieure à sa capacité de transport légère (100 kilogrammes), cela l’empêche d’utiliser son pouvoir de marche sur l’eau.
</div></td></tr></table>
