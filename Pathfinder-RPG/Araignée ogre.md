---
Name: Pathfinder-RPG.Araignée ogre
Title: Araignée, Araignée ogre
LastModified: 2020-09-18 02:45
Categories:
- Bestiaire 3
- Monstre
- Règles officielles
---

<table class="Bestiaire"><tr><td>
<div class="BD">
{s:BDTitre|Araignée ogre|FP 5}
{s:pucem|Vermine|Collines|Tempéré}
{s:BDTexte|*Source : Bestiaire 3 page 14*}
{s:BDTexte|**PX** 1 600}
{s:BDTexte|[[type Vermine|Vermine]] de taille TG, N}
{s:BDTexte|**Init** +2 ; **Sens** [[vision dans le noir]] à 12 cases, [[Perception des vibrations (capacité)|perception des vibrations]] à 12 cases ; [[Perception]] +5}
{s:BDSousTitre|Défense}
{s:BDTexte|**CA** 18, contact 10, pris au dépourvu 16 (Dex +2, naturelle +8, taille -2)}
{s:BDTexte|**pv** 52 (7d8+21)}
{s:BDTexte|**Réf** +4, **Vig** +8, **Vol** +3}
{s:BDTexte|**Immunité** effets mentaux}
{s:BDSousTitre|Attaque}
{s:BDTexte|**VD** 12 m (8 {s:c}), escalade 12 m (8 {s:c})}
{s:BDTexte|**Corps à corps** morsure, +8 (2d8+7 plus [[poison]])}
{s:BDTexte|**Espace occupé** 4,50 m (3 {s:c}) ; Allonge 4,50 m (3 {s:c})}
{s:BDTexte|**Attaque spéciale** [[toile (capacité)|toile]] (à distance +5, DD 16, pv 7)}
{s:BDSousTitre|Caractéristiques}
{s:BDTexte|**For** 21, **Dex** 15, **Con** 16, **Int** —, **Sag** 12, **Cha** 2}
{s:BDTexte|**BBA** +5, **BMO** +12, **DMD** 24 (36 contre le croc-en-jambe)}
{s:BDTexte|**Compétences** [[Discrétion]] -2, [[Escalade]] +29, [[Perception]] +5}
{s:BDTexte|**Modificateurs raciaux** +16 en [[Escalade]], +4 en [[Perception]], +4 en [[Discrétion]]}
{s:BDTexte|**Part** [[Compression (capacité)|compression]]}
{s:BDSousTitre|Écologie}
{s:BDTexte|**Environnement** collines ou souterrains froids ou tempérés}
{s:BDTexte|**Organisation** solitaire ou couple}
{s:BDTexte|**Trésor** fortuit}
{s:BDSousTitre|Pouvoirs spéciaux}
{s:BDTexte|**Poison (Ext).** Morsure — blessure ; JdS [[Vigueur]] DD 18 (incluant un bonus racial de +2) ; fréquence 1/round pendant 6 rounds ; effet affaiblissement temporaire de 1d4 For et de 1d4 Dex ; guérison 1 réussite.}
</div>
</td><td><div><div style="float:right; background-color: #fff; padding: 0 0 0 0; margin: 0 0 4px 8px">

<p class="tablo-caption">Variantes d'araignée</p>

{.tablo .autoalt}
| Nom | FP |
| --- | --- |
| Familier, [[Araignée écarlate]] | 1/4 |
| [[Araignée géante]] | 1 |
| [[Araignée goliath]] | 11 |
| [[Araignée lunaire]] | 2 |
| [[Araignée ogre]] | 5 |
| [[Araignée-crabe géante]] | 1/2 |
| [[Nuée daraignées|Nuée d'araignées]] | 1 |
| [[Tarentule géante]] | 8 |
| [[Veuve noire géante]] | 3 |

</div>*Cette énorme araignée a la taille d’un éléphant. Ses pattes présentent des articulations hérissées de piquants et ses traits sont vaguement et sinistrement humanoïdes.*

Les araignées ogres sont
des chasseresses sauvages et
terrifiantes qui tissent des
toiles complexes capables de
recouvrir des arbres entiers.
Ainsi nommées à cause de la
disposition de leurs yeux et
de leurs mandibules, qui les
fait ressembler de manière
déroutante à des ogres dont
elles ont quasiment la taille,
les araignées ogres peuvent
se faufiler dans des coins
et dans des tunnels bien
plus étroits qu’on ne
pourrait s’y attendre.
</div></td></tr></table>
