---
Name: Pathfinder-RPG.Réactifs alchimiques
Title: Réactifs alchimiques
LastModified: 2025-02-25 18:35
Categories:
- Src Adventurers Armory
---

{s:MenuEquipement}{s:cs}Les réactifs suivants sont purifiés par de longs processus à partir de leur état brut pour être utilisés dans des recettes alchimiques. Chaque réactif alchimique peut également être utilisé comme composante de puissance alchimique, augmentant les effets de certains sorts lorsqu'il est utilisé comme [[composantes|composante matérielle]] supplémentaire. Les composantes de puissance alchimique ont été introduites pour la première fois dans le Compagnon du joueur : Armurerie de l’aventurier (et sont décrites ci-dessous). L'utilisation d'un réactif comme composante de puissance alchimique nécessite un certain nombre de doses du réactif, n'affecte que les sorts qui répondent aux critères spécifiés et n'augmente qu'un effet que le sort produit déjà (par exemple, un personnage peut utiliser de la poudre noire comme composante de puissance alchimique uniquement pour un sort qui inflige des dégâts d’énergie destructive). Les réactifs ne se cumulent ni avec eux- mêmes ni entre eux et sont consommés après utilisation.

{s:AA}Une composante de puissance alchimique est un objet alchimique utilisé comme [[composantes|composante matérielle]] ou [[composantes|focalisateur]] d'un sort afin de modifier ou d'améliorer les effets normaux du sort. Ce qui suit est un échantillon de ces effets ; le [[MJ]] peut autoriser d'autres combinaisons. Les sorts suivis d'un (M) dépensent l'objet alchimique en tant que composante matérielle ; ceux suivis d'un (F) utilisent l'objet comme focalisateur et ne le dépensent donc pas. Dans les deux cas, l'objet alchimique ne produit pas son effet normal et n'affecte aucun autre paramètre du sort. Un personnage ne peut pas utiliser le même objet comme focalisateur et comme composante matérielle en même temps.
...
- **Brume acide (M).** Le rayon et la hauteur de la brume augmente de 1,5 m.
- **Dissipation de la magie (M).** Utiliser de l’acide comme composante matérielle en tentant de dissiper ou de contrer un sort ou un effet du sous-type de la terre offre un bonus de +2 au test de dissipation. Ce bonus s’applique aussi bien sur *[[dissipation de la magie]]*, *[[dissipation suprême]]* que sur tout autre sort utilisé pour effectuer un [[contresort]] (par exemple, si le personnage possède le don [[Science du contresort]]).
- **Flèche acide (M).** L’acide du sort dure 1 round supplémentaire.
- **Graisse (M).** La graisse est acide et inflige 1 point de dégâts d’acide par round à toute créature dans la zone d’effet ou tenant l’objet graissé.
- **Mur de glace (M).** Pour chaque flasque d’acide utilisée comme composante de puissance, le personnage peut désigner un carré de 3 m de côté du *mur de glace* qui, s’il est creusé, inflige 1d6 points de dégâts d’acide en plus des dégâts de froid.
...
- **Pyrotechnie (M).** Augmente le rayon du nuage de 1,5 m et sa durée de 2 [[round|rounds]].
...
- **Protection contre les énergies destructives (M).** S’il est lancé pour protéger du froid, augmente de 5 le nombre de dégâts de froid absorbés.
- **Rayon ardent (F).** Ajoute un bonus de circonstances de +1 au [[jet dattaque|jet d’attaque]] de tous les rayons lancés par le sort.
- **Rayon ardent (M).** Toutes les cibles blessées par un *rayon ardent* prennent feu comme si elle avait été touchée par un [[Armes alchimiques#Feu grégeois|feu grégeois]].
- **Résistance aux énergies destructives (M).** S’il est lancé pour protéger du froid, passe la résistance au froid à 12. À partir du [[NLS|niveau de lanceur de sorts]] 7, le personnage peut utiliser deux flasques de feu grégeois comme composante de puissance pour passer la résistance au froid à 24. À partir du [[NLS|niveau de lanceur de sorts]] 11, le personnage peut utiliser trois flasques de feu grégeois comme composante de puissance pour passer la résistance au froid à 36.
- **Sphère de feu (M).** Toutes les cibles blessées par la *[[sphère de feu]]* prennent feu comme si elle avait été touchée par un [[Feu grégeois (objet)|feu grégeois]].
...
- **Protection contre les énergies destructives (M).** S’il est lancé pour protéger de la chaleur, augmente de 5 le nombre de dégâts de feu absorbés.
- **Rayon de givre (F).** Le sort inflige +1 point de dégâts.
- **Rayon de givre (M).** Le sort crée un glaçon de vapeur d’eau gelée qui frappe la cible et inflige 1d3 points de dégâts perforants et 1 point de dégâts de froid.
- **Résistance aux énergies destructives (M).** S’il est lancé pour protéger du feu, passe la résistance au feu à 12. À partir du [[NLS|niveau de lanceur de sorts]] 7, le personnage peut utiliser deux jarres de glace liquide comme composante de puissance pour passer la résistance au feu à 24. À partir du [[NLS|niveau de lanceur de sorts]] 11, le personnage peut utiliser trois jarres de glace liquide comme composante de puissance pour passer la résistance au feu à 36.
...
- **Mur de pierre (M).** Pour chaque pot de graisse utilisé comme composante de puissance, le personnage peut désigner un carré de 3 m du *mur de pierre* qui devient glissant (+5 au [[DD]] d’[[Escalade]]).
- **Tempête de neige (M).** Augmente le [[DD]] d’[[Acrobaties]] de 1.
...
- **Répulsif (M).** La première vermine à entrer dans l’émanation est sujette aux effets du [[Poil à gratter (objet)|poil à gratter]] (DD 15). Chaque sachet de poil à gratter utilisé au-delà du premier permet au sort d’affecter une autre vermine entrant dans l’émanation. Une nuée de vermines compte comme une seule vermine dans le cadre de cet effet.
...
- **Toile d’araignée (M).** Augmente le [[DD]] pour se libérer en faisant un test de [[manoeuvres offensives|manoeuvre offensive]] ou d’[[Évasion]] de 1.
