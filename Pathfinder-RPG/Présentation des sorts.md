---
Name: Pathfinder-RPG.Présentation des sorts
Title: Présentation des sorts
LastModified: 2025-08-10 13:14
Categories:
- Magie
- Règle officielle
...
Les enchantements sont des sorts mentaux. Deux branches permettent d’influer sur la créature victime.

{s:Reference|CHARME}**Charme.** Le sort modifie la manière dont le sujet perçoit le lanceur de sorts, ce qui l’incite le plus souvent à considérer ce dernier comme un ami.
...
Si le personnage lance un sort de métamorphose sur une créature d’une catégorie de taille inférieure à Petite ou supérieure à Moyenne, il doit ajuster ses valeurs de caractéristique en fonction de la table suivante avant d’appliquer les bonus liés au sort de métamorphose.

<center>
{| CLASS="tablo"
|- CLASS="titre"
| Taille originale de la créature || For || Dex || Con || Nouvelle taille
|- CLASS="premier"
| Infime || +6 || -6 || — || Petite
|-
| Minuscule || +6 || -4 || — || Petite
|- CLASS="alt"
| Très petite || +4 || -2 || — || Petite
|-
| Grande || -4 || +2 || -2 || Moyenne
|- CLASS="alt"
| Très grande || -8 || +4 || -4 || Moyenne
|-
| Gigantesque || -12 || +4 || -6 || Moyenne
|- CLASS="alt"
...
La plupart des registres n’ont aucun effet sur le jeu, mais ils indiquent comment le sort interagit avec les autres sorts, les pouvoirs spéciaux, les créatures inhabituelles, les alignements, etc.

Les sorts de langage doivent forcément reposer sur une langue compréhensible. Le sort échoue si la cible ne comprend pas le personnage, soit parce qu’elle ne parle pas le langage utilisé, soit parce qu’elle ne peut pas l’entendre.
...
{s:Reference|PORTEE}
### La portée
La portée d’un sort, donnée dans sa description, indique quelle distance il peut atteindre. Il s’agit soit de la distance maximale qu’il peut affecter à partir du lanceur de sorts, soit de celle à laquelle on peut centrer son effet. Si une partie de la zone d’effet dépasse la portée maximale, le sort est sans effet sur cette partie. Les portées habituelles sont :
...
**Moyenne.** Le sort fonctionne jusqu’à 30 m (20 cases), plus 3 m (2 cases) par niveau de lanceur de sorts.
...
### Choisir sa cible
Selon le type de sort, il faut déterminer où il se matérialise ou qui il affecte. Cette ligne décrit donc la ou les cibles du sort, son effet ou sa zone d’effet.
...
{s:Reference|EFFET}
#### Effet
Certains sorts créent ou font apparaître des choses plutôt que d’affecter des éléments déjà présents.
...
{s:Reference|DEFEMANATION}Une *émanation* fonctionne comme un rayonnement, si ce n’est que son effet continue d’irradier depuis son point d’origine pendant toute la durée du sort. La plupart des émanations prennent la forme d’une sphère ou d’un cône.

{s:Reference|DEFETENDUE}Une *étendue* se propage comme un rayonnement mais elle peut franchir les angles. Le personnage sélectionne uniquement le point d’origine, et le sort s’étend dans toutes les directions sur une distance donnée. Pour estimer la distance parcourue, le personnage doit tenir compte des éventuels tournants pris par le sort.

**Cône, cylindre, ligne ou sphère.** La plupart des sorts qui affectent une zone ont une forme précise.

{s:Reference|DEFCONE}Un *cône* prend la forme d’un quart de cercle qui irradie depuis le lanceur de sorts. Il part de n’importe quel coin de la case du personnage puis s’élargit dans la direction indiquée par celui-ci. La plupart des cônes sont des rayonnements ou des émanations (voir ci-dessus) et ne franchissent donc pas les angles.

{s:Reference|DEFCYLINDRE}Quand le personnage lance un sort à effet *cylindrique*, il décide de son point d’origine. Ce point est le centre d’un cercle horizontal et le sort descend de ce cercle pour remplir un cylindre. Un sort de forme cylindrique ignore tous les obstacles de la zone.

{s:Reference|DEFLIGNE}Une *ligne* émane du personnage et s’étend dans la direction qu’il indique. Elle part de l’un des coins de sa case et s’étire jusqu’à atteindre la limite de portée du sort ou jusqu’à ce qu’un obstacle la bloque. Une ligne affecte toutes les créatures présentent dans les cases traversées.

{s:Reference|DEFSPHERE}Une *sphère* se développe depuis son point d’origine jusqu’à remplir une zone sphérique. Les sphères sont des rayonnements, des émanations ou des étendues.
...
Une barrière solide dans laquelle il y a un trou d’au moins 30 centimètres de côté  ne constitue plus un obstacle pour la ligne d’effet d’un sort. Une telle ouverture signifie que la zone de mur d’un mètre cinquante contenant le trou n’est plus considéré comme une barrière pour ce qui est de la ligne d’effet du sort.

{s:Reference|DUREE}
### La durée
L’entrée « Durée » du sort indique combien de temps l’énergie magique du sort dure.

**Durée déterminée.** Dans la plupart des cas, la durée se mesure en rounds, minutes, heures, ou autre unité de temps. Quand la durée indiquée arrive à son terme, la magie cesse de faire effet et le sort s’achève. Lorsque la durée varie, le MJ doit jeter les dés sans communiquer le résultat au lanceur, de façon à ce qu’il ne sache pas quand son sort s’achèvera.

**Instantanée.** L’énergie du sort se dissipe immédiatement, même si ses effets peuvent se prolonger.
...
Les créatures entrant dans la zone sont aussitôt affectées et ne le sont plus dès qu’elles en sortent.

**Retenir un sort de contact.** Si le personnage n’utilise pas son sort de contact au cours du round où il prononce l’incantation, il peut souvent retenir indéfiniment son énergie magique. À chaque round, il peut tenter une attaque de contact mais s’il lance un autre sort, l’énergie se dissipe aussitôt. Certains sorts de contact permettent de toucher plusieurs cibles. Le personnage ne peut pas retenir un tel sort et doit donc toucher les cibles durant le dernier round de l’incantation.
...
La plupart des sorts offensifs s’accompagnent généralement d’un jet de sauvegarde permettant à la cible d’éviter tout ou partie des effets. La ligne « Jet de sauvegarde » indique le type de jet de sauvegarde à effectuer et la manière dont il fonctionne contre le sort.

**Annule.** Le sort n’a aucun effet sur une créature qui réussit son jet de sauvegarde.

**Partiel.** Normalement, le sort a un effet donné sur la cible. Si cette dernière réussit son jet de sauvegarde, elle subit un effet moindre.
...
| 10e || Tout le reste
|-
| CLASS="note" COLSPAN="2" | *(*) Indique l’ordre dans lequel les objets risquent d’être affectés.*
|}
</center>
...
Sur cette ligne, les termes « objet » et « inoffensif » ont le même sens que pour les jets de sauvegarde. Une créature possédant une certaine résistance à la magie doit volontairement choisir d’abaisser sa résistance à la magie par une action simple si elle souhaite être affectée par le sort sans que son lanceur ne soit contraint de réussir un test de lanceur de sorts.

### Description
Cette partie décrit les effets et le fonctionnement du sort. Si l’une des lignes précédentes porte la mention « voir description », c’est ici que se trouve cette explication.
