---
Name: Pathfinder-RPG.Transfert de sorts
Title: Transfert de sorts
LastModified: 2026-04-28 15:20
Categories:
- Règle officielle
- Sort
- Sort chaman 4
- Sort prêtre 4
- Sort spirite 4
- Src Manuel des joueurs
---

**École** [Évocation](./Évocation.md) ; **Niveau** [Cham](./Chaman.md) 4, [Prê](./Prêtre.md) 4, [Spi](./Spirite.md) 4
**Temps d'incantation** 10 minutes
**Composantes** [V, G, FD](./composantes.md)
**Portée** [contact](./Présentation des sorts.md#PORTEE)
**Cible** créature touchée, voir texte
**Durée** [permanente](./Présentation des sorts.md#DUREE) jusqu’à utilisation ([T](./Présentation des sorts.md#DUREE))
**Jet de sauvegarde** [Volonté](./Volonté.md), [annule](./Présentation des sorts.md#JETSDESAUVEGARDE) (inoffensif) ; **Résistance à la magie** oui (inoffensif)
---
Au terme de l’incantation, le [prêtre](./Prêtre.md) transfère à la créature de son choix un ou plusieurs sorts qu’il a préparés. Le bénéficiaire, qui doit avoir un minimum de 5 en [Intelligence](./Intelligence.md) et de 9 en [Sagesse](./Sagesse.md), peut alors lancer ces sorts comme s’il les avait préparés normalement. Le lanceur ne peut transmettre que des [abjurations](./Abjuration.md), des [divinations](./Divination.md) et des [invocations](./Invocation.md) ([guérison](./guérison.md)) de cette manière. Le nombre de sorts transférés (et leur niveau) dépend uniquement du nombre de [DV](./../Ressources/DV.md) de la créature ; Le lanceur de sorts aura beau lancer *transfert de sorts* à plusieurs reprises il ne pourra pas dépasser cette limite.

{| CLASS="tablo centre autoalt"
|- CLASS="titre"
| DV du bénéficiaire || Sorts transférés
|- CLASS="premier"
| 2 ou moins || 1 sort du 1er [niveau](./Niveau de sort.md)
|-
| 3–4 || 1 ou 2 sorts du 1er [niveau](./Niveau de sort.md)
|-
| 5+ || 1 ou 2 sorts du 1er [niveau](./Niveau de sort.md) et 1 du 2e [niveau](./Niveau de sort.md)
|}

Les caractéristiques variables du sort (portée, durée, zone d’effet, etc.) sont calculées à partir du [niveau du lanceur de sorts](./NLS.md), pas du nombre de [DV](./../Ressources/DV.md) de la créature qui le reçoit.

Dès que le personnage a lancé *transfert de sorts*, il perd un sort du 4e [niveau](./Niveau de sort.md) tant que le bénéficiaire n’a pas lancé tous les sorts qu’il a reçus (ou tant qu’il est en vie). Son dieu tient le [prêtre](./Prêtre.md) pour responsable de l’utilisation qui est faite des sorts transmis. Si pour une raison ou pour une autre le quota de sorts du 4e [niveau](./Niveau de sort.md) du [prêtre](./Prêtre.md) diminue et tombe en dessous du nombre de *transferts de sorts* qu’il a lancés et qui sont actifs, les sorts qu’il a le plus récemment transférés disparaissent de l’esprit du bénéficiaire.

Pour lancer un sort accompagné d’une [composante verbale](./composante verbale.md), le bénéficiaire doit pouvoir parler. Si le sort nécessite une [composante gestuelle](./composante gestuelle.md), il doit avoir des mains [humanoïdes](./type Humanoïde.md). Enfin, s’il requiert une [composante matérielle](./composante matérielle.md) ou un [focaliseur](./focaliseur.md), il doit en avoir un à disposition.



((({s:sortssimilaires|transfert de sorts}
## [Transfert de puissance élémentaire](./Transfert de puissance élémentaire.md)
{s:racial|Suli (race)|sulis|Illustrations/PNJ/Suli.jpg}{s:MR}
**École** [Évocation](./Évocation.md) <nowiki>[</nowiki>[voir texte](./Registre.md)<nowiki>]</nowiki> ; **Niveau** [Cham](./Chaman.md) 2, [Ens](./Ensorceleur.md)/[Mag](./Magicien.md) 2, [Magus](./Magus.md) 2, [Prê](./Prêtre.md) 2, [San](./Sanguin.md) 2
**Temps d'incantation** 1 [action simple](./Action simple.md)
**Composantes** [V, G](./composantes.md)
**Portée** [contact](./Présentation des sorts.md#PORTEE)
**Cible** créature touchée, voir texte
**Durée** 24 heures ou jusqu'à déchargement ([T](./Présentation des sorts.md#DUREE))
**Jet de sauvegarde** [Volonté](./Volonté.md), [annule](./Présentation des sorts.md#JETSDESAUVEGARDE) (inoffensif) ; **Résistance à la magie** oui (inoffensif)
---
Ce sort fonctionne comme *[transfert de sorts](./Transfert de sorts.md)*, à la seule différence que le personnage transfère l’utilisation de sa capacité [assaut élémentaire](./suli (race).md) à la cible. La cible doit avoir un score d’[Intelligence](./Intelligence.md) d’au moins 5 pour utiliser cette capacité. L’[assaut élémentaire](./suli (race).md) transféré fonctionne exactement comme celui du personnage, à la seule différence que la durée de la capacité est basée sur le [niveau](./niveau.md) ou les [dés de vie](./dés de vie.md) de la cible. Une fois que le personnage a [lancé ce sort](./Lancer des sorts.md), il ne peut plus utiliser sa capacité [assaut élémentaire](./suli (race).md) tant que la durée du sort n’est pas terminée.
)))
